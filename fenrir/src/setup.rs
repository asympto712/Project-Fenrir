...
impl ModelSetupConfig {
    ...
    Result<(Vec<Vec<(P, VarStore)>>, Vec<OsString>)> {
        ...
        let filtered_infos = if self.use_mpi {
            self.module_load_infos
                .iter()
                .filter(|x| x.rank.is_some_and(|rank| rank == SimpleCommunicator::world().this_process().rank()))
                .collect::<Vec<_>>()
        } else {
            self.module_load_infos
                .iter()
                .filter(|x| x.rank.is_none())
                .collect::<Vec<_>>()
        };
        
        for ((name, model, vs), info) in loaded_modules.into_iter().zip(filtered_infos.iter()) {
            
            assert_eq!(rank, info.rank);
...
pub fn load_module<P: PVModel>(info: &ModuleLoadInfo) -> Result<(OsString, P, VarStore)> {
    ...
    match path.try_exists() {
        Ok(f) => {
            if f { // if path exists and is available
                let mut vs = VarStore::new(info.device);
                let module: P = P::new(&vs.root(), &info.config);
                vs.load(path)?;
                Ok((info.path.to_owned(), module, vs))
            } else {
                println!("Creating new model at {:?}", info.path);
                new_module(info.device, &info.config, &info.path)
            }
        },
        Err(_) => {
            Err(eyre!("Existence of File at {:?} could not be verified", path))
        }
    }
}
...
pub fn new_module<P: PVModel>(device: Device, model_config: &ModelConfig, filename: &OsString) -> Result<(OsString, P, VarStore)> {
    ...
    let module: P = P::new(&vs.root(), model_config);
    vs.save(filename.to_owned());
    Ok((filename.to_owned(), module, vs))
}
    ...
    if rank == 0 {
        ...
        for module_info in config.module_load_infos.iter() {
            ...
            if module_info.rank.is_some_and(|i| i == 0) {
                ...
                // Since root is responsible for answering for the model requests from other nodes,
                // it initializes any model whose path does not exist, even if that model is not to be loaded in it.
                let _ = new_module::<P>(Device::Cpu, &module_info.config, &module_info.path)?;
            }
        }
    }

    world.barrier(); // Wait for the root node to finish loading
...
fn request_model_from_root(world: &SimpleCommunicator, root: i32, path: &str) -> Cursor<Vec<u8>> {
    ...
    // Send request
    world.process_at_rank(root).send_with_tag(path.as_bytes(), TAG_MODEL_REQUEST);
