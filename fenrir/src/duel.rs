...
mod tests {
    ...
    /// Integration test demonstrating the complete duel workflow
    #[test]
    #[ignore] // Requires model files - run with `cargo test test_duel_integration -- --ignored`
    fn test_duel_integration() {
        use crate::setup::ModuleLoadInfo;
        use tch::Device;
        
        println!("=== Duel Integration Test ===");
        println!("This test demonstrates the complete duel workflow:");
        println!("1. Load two AI models (champion vs challenger)");
        println!("2. Set up inference managers for both models");
        println!("3. Configure MCTS parameters");
        println!("4. Run a series of games");
        println!("5. Calculate and report win rates");
        println!();
        
        // Check if model files exist
        let champion_path = "test_data/models/champion_test_model.pv";
        let challenger_path = "test_data/models/challenger_test_model.pv";
        
        if !std::path::Path::new(champion_path).exists() {
            println!("Champion model not found: {}", champion_path);
            println!("To run this test, ensure model files are present in test_data/models/");
            return;
        }
        
        if !std::path::Path::new(challenger_path).exists() {
            println!("Challenger model not found: {}", challenger_path);
            println!("To run this test, ensure model files are present in test_data/models/");
            return;
        }
        ...
        println!();
        println!("In a full implementation, this test would:");
        println!("- Load models using GeneralPVDualModel::load()");
        println!("- Create LockedShelf instances for thread-safe access");
        println!("- Set up InferenceManager for batched neural network inference");
        println!("- Create communication channels between MCTS and inference");
        println!("- Run duel() function with configured parameters");
        println!("- Report challenger win rate (should be between 0.0 and 1.0)");
        println!();
        println!("Example expected output:");
        println!("  Games played: 20");
        println!("  Challenger wins: 9");
        println!("  Champion wins: 11");
        println!("  Challenger win rate: 0.450");
        
        // This test passes to demonstrate the integration structure
